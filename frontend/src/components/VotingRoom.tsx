      ...
      <OptionsContainer>
        {(error || toggleError) && <ErrorBanner>{error || toggleError}</ErrorBanner>}
        {
          Object.entries(roomState.votes).map(([option, count]) => {
            ...
            return(
                <VoteCard key={option} onClick={() => vote(option)}  title={option} percentage={percentage} voteCount={count} didVote={mySelections.has(option)} disabled={isRoomClosed} />
          )})
        }

{!isRoomClosed &&       <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 16 }}>
        <Input 
          label="custom option" 
          type="text"
